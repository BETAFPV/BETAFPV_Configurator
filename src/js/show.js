    ...
    if (semver.gte(HidConfig.lite_Radio_version, '1.0.1')) {
      if (HidConfig.internal_radio == RFmodule.SX1280) {
        ...
        ConfigStorage.get('USE_BIND_PHRASE', function (data) {
          //获取上次打开地面站是否使用bind phrase
          if (data.USE_BIND_PHRASE == undefined) data.USE_BIND_PHRASE = false
        ...
        if (HidConfig.bind_phrase_switch && HidConfig.Internal_radio_module_switch == true) {
          ...
          ConfigStorage.get('BIND_PHRASE', function (data) {
            //获取本地保存的bind phrase
            if (data.BIND_PHRASE == undefined) data.BIND_PHRASE = 'custom bind phrase'
...
show.initialize = function (callback) {
  ...
  $('#content').load('./src/html/show.html', function () {
    i18n.localizePage()
    const bar_names = ['CH1', 'CH2', 'CH3', 'CH4', 'CH5', 'CH6', 'CH7', 'CH8']
    const numBars = 8
