...
const FC = {
  ...
  getSerialRxTypes            : () => {
    ...
    // defaults
    const serialRxTypes = ['SPEKTRUM1024', 'SPEKTRUM2048', 'SBUS', 'SUMD', 'SUMH', 'XBUS_MODE_B', 'XBUS_MODE_B_RJ01']

    if (semver.gte(apiVersion, '1.15.0')) {
      serialRxTypes.push('IBUS')
    }

    if ((flightControllerIdentifier === 'BTFL' && semver.gte(flightControllerVersion, '2.6.0')) || (flightControllerIdentifier === 'CLFL' && semver.gte(apiVersion, API_VERSION_1_31))) {
  ...
  getPidDefaults() {
    ...
    if (semver.gte(this.CONFIG.apiVersion, API_VERSION_1_43)) {
      versionPidDefaults = [42, 85, 35, 23, 90, 46, 90, 38, 25, 95, 45, 90, 0, 0, 90]
    }
